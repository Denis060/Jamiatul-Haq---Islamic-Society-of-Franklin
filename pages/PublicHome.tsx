...
import { MOCK_PRAYER_TIMES, MOCK_PROFILE } from '../services/supabase';
      ...
      <section className="relative min-h-[90vh] flex items-center justify-center text-white overflow-hidden bg-[#042f24]">
        ...
        <div className="relative z-10 max-w-5xl mx-auto px-4 text-center">
          ...
          <p className="text-xl md:text-3xl font-arabic mb-10 text-[#d4af37] tracking-[0.2em] uppercase">
            Islamic Society of Franklin Township
          </p>
        ...
        <div className="absolute bottom-10 left-1/2 -translate-x-1/2 bg-[#d4af37] text-[#042f24] px-8 py-3 rounded-full font-bold shadow-2xl animate-bounce flex items-center gap-2">
          <Heart size={18} fill="currentColor" /> Jumu'ah Prayer: {MOCK_PROFILE.jumua_time}
        </div>
      </section>
      ...
      <section className="py-24 bg-[#fdfbf7] relative">
        <div className="max-w-7xl mx-auto px-4">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-5 gap-6">
            {[
              { label: 'Fajr', time: MOCK_PRAYER_TIMES.fajr, icon: 'dawn' },
              { label: 'Dhuhr', time: MOCK_PRAYER_TIMES.dhuhr, icon: 'noon' },
              { label: 'Asr', time: MOCK_PRAYER_TIMES.asr, icon: 'afternoon' },
              { label: 'Maghrib', time: MOCK_PRAYER_TIMES.maghrib, icon: 'sunset' },
              { label: 'Isha', time: MOCK_PRAYER_TIMES.isha, icon: 'night' },
            ].map((p, idx) => (
      ...
      <section className="py-24 bg-[#042f24] text-white overflow-hidden relative">
        ...
        <div className="max-w-7xl mx-auto px-4 relative z-10">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
            {[
              { type: 'Sacred Month', title: 'Ramadan 2024 Prep', desc: 'Join us for pre-Ramadan cleanups and lecture series starting this weekend.', date: 'Mar 01' },
              { type: 'Community', title: 'Monthly Youth Halaqa', desc: 'Focusing on identity and faith in the modern world. Every first Saturday.', date: 'Feb 25' },
              { type: 'Expansion', title: 'Masjid Building Fund', desc: 'We have reached 60% of our goal. Help us finalize the new wing.', date: 'Ongoing' }
            ].map((item, idx) => (
              <div key={idx} className="bg-white/5 backdrop-blur-sm border-t-4 border-[#d4af37] p-10 rounded-b-[3rem] hover:bg-white/10 transition-all flex flex-col h-full">
                <span className="text-[#d4af37] font-black text-xs uppercase tracking-[0.2em] mb-4">{item.type}</span>
                <h3 className="text-2xl font-bold mb-4">{item.title}</h3>
                <p className="text-white/70 leading-relaxed mb-8 flex-grow">{item.desc}</p>
