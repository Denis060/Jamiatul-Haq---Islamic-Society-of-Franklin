...
import {
  Plus, Trash2, Edit2, Camera, Image as ImageIcon, Loader2,
...
const AdminGallery = () => {
  ...
  const [newAlbum, setNewAlbum] = useState({ title: '', description: '', cover_image: null as File | null, existing_cover_url: '' });
  ...
  const handleSaveAlbum = async (e: React.FormEvent) => {
    ...
    try {
      let coverUrl = newAlbum.existing_cover_url;
      if (newAlbum.cover_image) {
        coverUrl = await uploadFile(newAlbum.cover_image, 'gallery-covers') || newAlbum.existing_cover_url;
      }
      ...
      let resultError;
      if (editingAlbumId) {
        const { error } = await supabase.from('gallery_albums').update(payload).match({ id: editingAlbumId });
        resultError = error;
      } else {
      ...
      setSuccess(editingAlbumId ? "Album updated successfully, Alhamdulillah!" : "Album created successfully, Alhamdulillah!");
      resetForm();
      fetchAlbums();
    } catch (err: any) {
      console.error("Album Save Error:", err);
      setError(err.message || "Failed to save album. Please check your connection.");
    } finally {
      setUploading(false);
  ...
  const startEditAlbum = (album: any) => {
    ...
    setNewAlbum({
      ...
      cover_image: null,
      existing_cover_url: album.cover_image_url || ''
    });
  ...
  const resetForm = () => {
    setNewAlbum({ title: '', description: '', cover_image: null, existing_cover_url: '' });
  ...
  return (
    <div className="max-w-6xl mx-auto pb-20">
      ...
      {isAddingAlbum && (
        <form onSubmit={handleSaveAlbum} className="bg-white p-12 rounded-[3.5rem] shadow-2xl border-2 border-[#f0e6d2] mb-16 animate-in zoom-in-95 duration-300">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
            ...
            <div>
              ...
              >
                ...
                ) : newAlbum.existing_cover_url ? (
                  <div className="relative w-full h-full group/img">
                    <img src={newAlbum.existing_cover_url} alt="Current Cover" className="w-full h-full object-cover opacity-50 group-hover:opacity-30 transition-opacity" />
                    <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
                      <Camera size={48} className="mb-4 text-[#d4af37] drop-shadow-md" />
                      <span className="font-black uppercase text-[10px] tracking-widest text-[#042f24] bg-white/80 px-4 py-2 rounded-full backdrop-blur-sm">Change Cover</span>
                    </div>
                  </div>
                ) : (
                  <>
          ...
          >
            {uploading ? 'Bismillah... Saving' : editingAlbumId ? 'Save Changes' : 'Create Album'}
          </button>
        </form>
      )}
            ...
            <div key={album.id} className="bg-white rounded-[3rem] border-2 border-[#f0e6d2] overflow-hidden group hover:shadow-2xl transition-all flex flex-col">
              <div className="relative aspect-[4/3] bg-slate-100 overflow-hidden">
                ...
                <div className="absolute top-6 right-6 flex gap-2">
                  <button onClick={(e) => { e.stopPropagation(); startEditAlbum(album); }} className="p-3 bg-white/90 backdrop-blur text-[#042f24] rounded-2xl shadow-xl hover:bg-[#d4af37] transition-all">
                    <Edit2 size={18} />
                  </button>
                  <button onClick={(e) => { e.stopPropagation(); deleteAlbum(album.id); }} className="p-3 bg-red-500/90 backdrop-blur text-white rounded-2xl shadow-xl hover:bg-red-600 hover:scale-110 transition-all">
                    <Trash2 size={18} />
                  </button>
                </div>
              </div>
